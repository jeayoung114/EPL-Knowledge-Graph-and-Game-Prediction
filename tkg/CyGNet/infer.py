...
n_batch = (test_data.shape[0] + 3 - 1) // 3

result_file = open('result.txt', 'w')
for idx in range(n_batch):
    batch_start = idx * 3
    batch_end = min(test_data.shape[0], (idx + 1) * 3)
    ...
    _, indices = torch.sort(test_score, dim=1, descending=True)
    indices = torch.nonzero(indices == test_label.view(-1, 1))
    indices = indices[:, 1].view(-1)

    picked = torch.argmin(indices).item()
    result_file.write(str(test_batch_data[picked][0]) + '\t' + str(test_batch_data[picked][1]) + '\t' + str(test_batch_data[picked][2]) + '\t' + str(test_batch_data[picked][3]))
    result_file.write('\n')
    print(test_batch_data[picked])
