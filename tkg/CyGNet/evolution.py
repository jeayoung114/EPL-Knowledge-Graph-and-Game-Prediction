...
def calc_filtered_test_mrr(num_entity, score, train_triplets, valid_triplets, valid_triplets2, test_triplets, entity, hits=[]):
    with torch.no_grad():
        ...
        hits1 = torch.mean((ranks <= hits[0]).float())
        hits3 = torch.mean((ranks <= hits[1]).float())
        hits5 = torch.mean((ranks <= hits[2]).float())
        hits10 = torch.mean((ranks <= hits[3]).float())
        hits15 = torch.mean((ranks <= hits[4]).float())

    return mrr.item(), hits1.item(), hits3.item(), hits5.item(), hits10.item(), hits15.item()
